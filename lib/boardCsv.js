const WEBSITE_MODEL_NUMBERS = [13, 8, 12, 6, 4, 5]
const MIN_WEBSITE_BET_PAYOUT = 1.10
const MODEL13_MIN_DISPLAY_PAYOUT = 1.05
...
function getModel(row, modelNumber) {
  ...
  const meetsPayoutFloor = oddsNumber >= (modelNumber === 13 ? MODEL13_MIN_DISPLAY_PAYOUT : MIN_WEBSITE_BET_PAYOUT)
...
function isBestBetEligible(model) {
  const minPayout = model.number === 13 ? MODEL13_MIN_DISPLAY_PAYOUT : MIN_WEBSITE_BET_PAYOUT
  return model.oddsNumber >= minPayout && model.oddsNumber <= MAX_WEBSITE_BEST_BET_PAYOUT
}
...
function parseCSV(text) {
  ...
  function parseSection(headerIdx) {
    ...
    for (let i = headerIdx + 1; i < lines.length; i++) {
      ...
      const selected = bestModel || (resultSection ? {
