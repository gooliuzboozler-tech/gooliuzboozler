...
function matchupFields(play, game) {
  const pitcherTeam = cleanTeam(play['Pitcher Team'])
  const teams = gameTeams(game)
  const prefix = pitcherTeam && teams.away === pitcherTeam ? 'at' : 'vs'
  return {
    'Matchup Prefix': prefix,
    'Matchup Label': `${prefix} ${play.Opponent || ''}`.trim(),
  }
}
...
async function enrichPlaysWithLiveMlb(plays, options = {}) {
  ...
  return plays.map(play => {
    ...
    const matchup = matchupFields(play, game)
    const feed = liveData.feeds.get(game.gamePk)
    if (!feed) return { ...play, ...matchup, 'GamePk': String(game.gamePk) }
